...
class ShibAuthToken implements IAuthentication {
    ...
    private function getAttributesInitToken(){
        ...
        else if($idp == 'https://core-infra-proxy.dev.eosc.grnet.gr/auth/realms/core'){
            if( empty($_SERVER['voPersonID'])){
                die('Did not receive required voPersonID attribute from the EOSC EU Node AAI (Testing) to complete authentication, please contact gocdb-admins');
            }
            if(empty($_SERVER['entitlement'])){
                die('Did not receive the required entitlement attribute from the EOSC EU Node AAI (Testing), please contact gocdb-admins');
            }
            $entitlementValuesArray = explode(';', $_SERVER['entitlement']);
	    /*
	     * TODO
	     ...
	     */
            $this->principal = $_SERVER['voPersonID'];
            $this->userDetails = array('AuthenticationRealm' => array('EOSC'));
            return;
        }
    }
